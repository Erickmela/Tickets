...
const fetchDatos = async (p = form.value.page) => {
    ...
    try {
        ...
        datos.value = [];
        const response = await categoriasStore.fetchCategorias(
            form.value.page,
        ...
        toastGlobalHelper.error(error.response?.data?.message || 'Error al cargar categorías');
    } finally {
        setTimeout(() => {
...
<template>
    <AdmLayout>
        ...
        <HeaderSecction title="Administración de Categorías" description="Gestiona todas las categorías de eventos">
            <template #actions>
        ...
        <Toolbar @searchText="form.filterSearch = $event" @itemsPerPage="form.filterItemsPage = $event"
            ...
            placeholder="Buscar categorías por nombre o estado...">
