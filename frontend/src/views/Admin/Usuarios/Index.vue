...
const fetchDatos = async (p = form.value.page) => {
    ...
    try {
        ...
        const response = await usuariosStore.fetchClientes(
            form.value.page,
        ...
        toastGlobalHelper.error(error.response?.data?.message || 'Error al cargar clientes');
    } finally {
        setTimeout(() => {
...
<template>
    <AdmLayout>
        ...
        <HeaderSecction title="Administración de Clientes"
            description="Gestiona todos los clientes que compran tickets">
            <template #actions>
