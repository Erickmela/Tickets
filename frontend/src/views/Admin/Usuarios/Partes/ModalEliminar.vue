...
const eliminarData = async () => {
    ...
    try {
        await usuariosStore.deleteCliente(props.data.id);
        toastGlobalHelper.success("Cliente eliminado exitosamente");
        emit("data_destroyed");
        closeModal();
    } catch (error) {
            ...
            <div class="space-y-4">
                ...
                <div v-if="data" class="p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
                    <div class="space-y-2">
                        ...
                        <div>
                            <span class="text-sm font-medium text-gray-700 dark:text-gray-300">DNI:</span>
                            <p class="text-gray-900 dark:text-white">{{ data.dni }}</p>
                        </div>
                        <div v-if="data.email">
                            <span class="text-sm font-medium text-gray-700 dark:text-gray-300">Email:</span>
                            <p class="text-gray-900 dark:text-white">{{ data.email }}</p>
                        </div>
                    </div>
                </div>
        ...
        <template #footer>
            <div class="flex justify-end space-x-2">
                ...
                <ButtonSave @click="eliminarData" :disabled="cargando" class="!bg-red-600 hover:!bg-red-700">
                    ...
                    <span v-else>Eliminar Cliente</span>
                </ButtonSave>
            </div>
        </template>
    </DialogModal>
